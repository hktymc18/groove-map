# セミナー受付・稼働率管理システム 設計書 v2（GROOVE CHECK-IN）
...
- **名簿**: 自ユニオンのメンバー一覧（検索・追加・編集・無効化）、
  CSVインポート（番号,氏名,系列[,かな,性別]）/エクスポート
- **イベント**: 月送りで一覧（日付・名称・出席数・合同バッジ）、作成
  （日付・名称・合同ユニオンのチェック選択）、削除、「受付を開始」
- **集計**: 月送りで自ユニオンの 開催数/のべ出席/平均稼働率、系列別のべ出席、
  メンバー×イベントの出席マトリクス+稼働率、CSV出力

オーナーはヘッダーのユニオン切替で全ユニオンを閲覧・操作可能。
